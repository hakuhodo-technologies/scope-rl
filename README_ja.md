...
- **オフ方策評価(OPE)**：OPEは，オフラインのログデータのみを使用して反実的方策の性能を評価することを目的としています．SCOPE-RLは多くのOPEの推定量をサポートし，推定量を評価し比較する合理的な実験手順を踏ます．さらに，状態-行動密度推定や累積分布推定に基づく推定量など，発展的なOPE手法も実装しています．

- **オフ方策選択(OPS)**：OPSは，オフラインのログデータを使用して，いくつかの候補方策の中から最も性能の良い方策を特定することを目的としています．SCOPE-RLはいくつかの基本的なOPS手法をサポートし，OPSの精度を評価するためのいくつかの指標を提供します．

このソフトウェアは，文脈付きバンディットにおけるOPEのためのライブラリである[Open Bandit Pipeline](https://github.com/st-tech/zr-obp)を参考にしています．SCOPE-RLはOPEについての実験を容易にするための一連のOPE推定量と手法を実装しており，強化学習だけでなく文脈付きバンディットの設定でも利用できます．
