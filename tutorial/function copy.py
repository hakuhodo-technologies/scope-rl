...
def train_behavior_policy(
    ...
    path_behavior_policy = Path(path_ / f"behavior_policy_{env_name}_{env.step_per_episode}.pt")
...
def obtain_logged_dataset(
    ...
    path_train_logged_dataset = Path(
        path_
        / f"train_logged_dataset_{env_name}_{behavior_policy_name}_{env.step_per_episode}.pkl"
    )
    path_test_logged_dataset = Path(
        path_
        / f"test_logged_dataset_{env_name}_{behavior_policy_name}_{env.step_per_episode}.pkl"
    )
    ...
    if path_train_logged_dataset.exists():
        ...
        test_logged_dataset = dataset.obtain_episodes(
            ...
            n_trajectories=n_trajectories,
            obtain_info=False,
            path=log_dir + f"/logged_dataset/multiple/{env.step_per_episode}",
...
def train_candidate_policies(
    ...
    path_candidate_policy = Path(
        path_ / f"candidate_policy_{env_name}_{env.step_per_episode}.pkl"
    )
...
def off_policy_evaluation(
    ...
    path_.mkdir(exist_ok=True, parents=True)
    path_input_dict = Path(path_ / f"input_dict_{env_name}_{env.step_per_episode}.pkl")
    ...
    if path_input_dict.exists():
        ...
        input_dict = prep.obtain_whole_inputs(
            ...
            path=log_dir + f"/input_dict/multiple/{env.step_per_episode}",
            random_state=base_random_state,
        )
